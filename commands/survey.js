...
module.exports = {
	...
	async execute(interaction) {
		...
		interaction.reply('Welcome to the !survey command. What would you like to ask the community?')
			...
			.then(() => {
				...
				const collector = interaction.channel.createMessageCollector({ filter, options });

				return interaction.followUp("test1");
			...
			.then(() => {
				...
				return interaction.followUp(filter, options);
			})
			.then((collected) => {
			...
			.then((collected) => {
				...
				return interaction.followUp(surveyEmbed)
			})
			...
			.then(collected => {
				...
				interaction.followUp(surveyResultsEmbed);
			})
			.catch((err) => {
				console.error('Something went wrong', err);
				if (err === 'timeout_format_error') {
					interaction.followUp("That doesn't seem to be a valid number...");
				} else {
